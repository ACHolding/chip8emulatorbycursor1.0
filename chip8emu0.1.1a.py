...
USER_GUIDE = """
chip8emulator by cursor
================================================================================

A lightweight, visually polished CHIP-8 emulator with an mGBA-inspired GUI
built in Python using Tkinter.

Screenshot placeholder (replace with a real capture when you have one):
  https://via.placeholder.com/640x320/000000/4da6ff?text=CHIP-8+Emulator+Screenshot

Features
--------------------------------------------------------------------------------
  • Accurate CHIP-8 CPU core
  • Dark mGBA-style interface (menu bar, toolbar, framed display, status bar)
  • Built-in demo ROM (sprite bounce)
  • Load external .ch8 / .rom / .c8 files
  • Configurable emulation speed (1–40 instructions per frame)
  • Sound via system bell when the sound timer is active
  • FPS hint in the status area (60 Hz tick when running)
  • Keyboard mapping aligned with the original CHIP-8 keypad layout
  • Reset, pause/resume, reload last ROM
...
--------------------------------------------------------------------------------
  ...
  From this directory:
    ...
    python3 chip8emu0.1.1bycursor.py

  Or rename the file to chip8emulator.py and run:

    python3 chip8emulator.py
""".strip()
...
TITLE = "chip8emulator by cursor 0.1"
DISPLAY_WIDTH = 64
DISPLAY_HEIGHT = 32
SCALE = 10
PROGRAM_START = 0x200
...
# mGBA-inspired dark shell + blue hue
BG = "#000000"
BUTTON_BG = "#000000"
PANEL = "#050508"
FRAME = "#0c1018"
...
def build_demo_rom() -> bytes:
    """Minimal sprite bounce demo (no external ROM)."""
    code = bytes([
        0x00, 0xE0,
        0x60, 0x0C,
        0x61, 0x08,
...
class Chip8App:
    ...
    def _show_about(self) -> None:
        messagebox.showinfo(
            TITLE,
            f"{TITLE}\n\nCHIP-8 emulator — mGBA-style layout (menus, toolbar, framed screen, status bar).\n\n"
            "Open Help → User Guide for full documentation.",
        )
